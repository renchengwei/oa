...
<script type="text/javascript">
	...
	$(document).ready(function(){
		postcommit('EmailAction.do?method=getReceiveEmailById',{id:id},'',function(result){
	...
	function back() {
		window.location.href="<%=basePath%>jsp/email/receiveemaillist.jsp";
	}
	
	//转发email
	function transmitemail() {
		window.location.href="<%=basePath%>jsp/email/newemail.jsp?flag=transmit&id=" + email.sendemailid;
	}
	//回复邮件
	function replyReceiveEmail() {
		window.location.href="<%=basePath%>jsp/email/newemail.jsp?flag=reply&id=" + email.id;
	}
	...
	function deleteemail() {
 		$.messager.confirm('确认','邮件删除后不可恢复,您确认想要删除记录吗？',function(r){    
		    if (r){ 
		    	postcommit('EmailAction.do?method=deleteReceiveEmails',{ids:id},'',function(result){
...
<body style="margin-top: 1px;margin-left: 5px;margin-right: 7px;">
	<div class="easyui-panel" nohead="true" style="overflow-x:hidden;">
		<div style="height: 30px;padding: 5px;margin: 0px;background-color: #D4D4D4;">
			...
			<a href="javascript:void(0)" class="easyui-linkbutton c2" style="padding-left: 5px;padding-right: 5px;margin-top: 0px;"
			 onclick="replyReceiveEmail();">回复邮件</a>
...
<!-- 			<a href="javascript:void(0)" class="easyui-linkbutton" onclick="replyReceiveEmail()" data-options="plain:false">回复</a> -->
...
<!-- 			<a href="javascript:void(0)" class="easyui-linkbutton" onclick="deleteemail()" data-options="plain:false">删除</a> -->
		...
		<div style="width:100%;background-color:#EBEBEB;padding: 10px;">
			...
			<div>
				<font color="green" style="line-height: 1.5">发件人：</font>
				<font id="senduser"></font>
			</div>
			<div>
				<font color="green" style="line-height: 1.5">时&nbsp;&nbsp;&nbsp;&nbsp;间：</font>
				<font id="sendtime"></font>
			</div>
			<div>
				<font color="green" style="line-height: 1.5">收件人：</font>
