...
<script type="text/javascript">
	...
	function onClickRow(rowIndex, rowData) {
		window.location.href="<%=basePath%>jsp/task/showsendtask.jsp?id=" + rowData.id;
	}
	
	function initdg() {
		$('#dg').datagrid({
	       	url:'TaskAaction.do?method=getSendTasksByPage',
			pagination:true,
			rownumbers:true,
			pageSize:20,
			pageList:[10,20,30],
			...
			columns:[[
				...
				{field:'name',title:'负责人',width:'10%',
					formatter: function(value,row,index){
						if(row.transactorbean) {
							return row.transactorbean.name;
						}
						return "";
					}
				},
